...
//-----------------
        ...
        if(block[set][way].valid){
                              invalidate(block[set][way].address);
...
uint8_t CACHE::invalidate(uint64_t address)
{
    uint8_t upper_level_dirty = 0;

    if (upper_level_icache[cpu]) {
        upper_level_dirty = ((CACHE *)upper_level_icache[cpu])->invalidate(address);

        if (upper_level_dcache[cpu] != upper_level_icache[cpu]) {
            upper_level_dirty = ((CACHE *)upper_level_dcache[cpu])->invalidate(address);
        }
    }
